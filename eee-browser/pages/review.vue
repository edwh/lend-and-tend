    ...
    <main class="max-w-7xl mx-auto px-4 py-8">
        ...
        <div class="flex gap-3 flex-wrap mb-6">
          ...
          >
            <div class="flex items-center gap-2">
              ...
              <span class="text-xs bg-gray-700 px-2 py-0.5 rounded">{{ field.labelled }}/{{ field.total }}</span>
              <span class="text-xs text-gray-500">w{{ field.weight }}</span>
            </div>
          </button>
        </div>

        <!-- Progress bar for selected field -->
        <div class="bg-gray-900 rounded-lg border border-gray-800 p-3">
          <div class="flex items-center justify-between mb-2">
            <span class="text-xs text-gray-500 uppercase tracking-wider">Progress</span>
      ...
      <template v-else-if="currentItem.messageid">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          <!-- Left: Image (takes up 2/3 on wide screens) -->
          <div class="lg:col-span-2">
            <div class="bg-gray-900 rounded-2xl border border-gray-800 overflow-hidden h-full flex flex-col">
              <div class="flex-1 bg-black flex items-center justify-center min-h-96">
                ...
                <div
                  v-if="imageFailed"
                  class="w-full h-96 bg-gray-800 flex items-center justify-center text-gray-500"
                >
          ...
          <!-- Right: Item info, question, and classification -->
        ...
        <!-- Accuracy table (shown if any field has ≥10 labels) -->
            ...
            <table class="w-full text-sm">
              <thead>
                <tr class="border-b border-gray-800 text-xs text-gray-500 uppercase tracking-wider">
                  ...
                  <th class="text-center px-4 py-3 font-medium">Labelled</th>
                  <th v-for="model in allModels" :key="model" class="text-center px-4 py-3 font-medium">{{ model }}</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="field in stats.fields" :key="field.field" class="border-t border-gray-800/60">
                  ...
                  <td class="px-4 py-3 text-center text-gray-300">{{ field.labelled }}</td>
...
const FIELDS: Field[] = [
  { field: 'EEE', short: 'EEE', weight: 5, dbColumn: 'is_eee', type: 'binary', labelled: 0, total: 0 },
  { field: 'Electrical components', short: 'Electrical', weight: 4, dbColumn: 'electrical_components_description', type: 'correct', labelled: 0, total: 0 },
  { field: 'Photo quality', short: 'Photo', weight: 4, dbColumn: 'photo_quality', type: 'correct', labelled: 0, total: 0 },
  { field: 'Condition', short: 'Condition', weight: 3, dbColumn: 'condition', type: 'correct', labelled: 0, total: 0 },
  { field: 'Weight (kg)', short: 'Weight', weight: 3, dbColumn: 'weight_kg_min', type: 'bucket', buckets: WEIGHT_BUCKETS, labelled: 0, total: 0 },
  { field: 'Value band', short: 'Value', weight: 2, dbColumn: 'value_band_gbp', type: 'correct', labelled: 0, total: 0 },
  { field: 'Brand', short: 'Brand', weight: 1, dbColumn: 'brand', type: 'correct', labelled: 0, total: 0 },
]
...
const fieldProgress = computed(() => {
  const field = fields.value.find((f) => f.field === selectedField.value)
  return field ? { labelled: field.labelled, total: field.total } : { labelled: 0, total: 0 }
})

const allModels = computed(() => {
  const models = new Set<string>()
  for (const field of stats.value.fields) {
    if (field.modelAccuracy) {
      Object.keys(field.modelAccuracy).forEach((m) => models.add(m))
    }
  }
...
const hasAnyLabels = computed(() => {
  return stats.value.fields.some((f) => f.labelled >= 10)
})
...
const loadNextItem = async () => {
  ...
  try {
    ...
    if (!nextResp.ok) throw new Error(`API error: ${nextResp.statusText}`)
    if (!statsResp.ok) throw new Error(`Stats error: ${statsResp.statusText}`)

    currentItem.value = await nextResp.json()
    const statsData = await statsResp.json()

    // Update fields with stats
    stats.value = statsData
    for (const statField of statsData.fields) {
      const field = fields.value.find((f) => f.field === statField.field)
      if (field) {
        field.labelled = statField.labelled
        field.total = statField.total
      }
    }
  } catch (e) {
...
const submitLabel = async (label: string) => {
  ...
  try {
    ...
    // Flash confirmation for 1.5 seconds, then load next
    setTimeout(() => {
