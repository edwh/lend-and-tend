...
func handlePromise(c *fiber.Ctx, myid uint64, req PostMessageRequest) error {
	...
	if req.Terms != nil {
		db.Exec("REPLACE INTO messages_promises (msgid, userid, terms) VALUES (?, ?, ?)", req.ID, promisedTo, string(*req.Terms))
	} else {
...
// handleAcceptAgreement records a tender accepting a lender's promise, sealing the agreement.
// The tender calls this; it sets acceptedat=NOW() on the existing promise where userid=myid.
func handleAcceptAgreement(c *fiber.Ctx, myid uint64, req PostMessageRequest) error {
	db := database.DBConn

	result := db.Exec(
		"UPDATE messages_promises SET acceptedat = NOW(), acceptedby = ? WHERE msgid = ? AND userid = ? AND acceptedat IS NULL",
		myid, req.ID, myid,
	)
	if result.RowsAffected == 0 {
		return fiber.NewError(fiber.StatusNotFound, "No pending agreement found for this user on this listing")
	}
