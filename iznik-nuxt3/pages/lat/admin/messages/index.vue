      ...
      <table class="table table-striped">
        ...
        <tbody>
          <tr v-for="msg in filteredMessages" :key="msg.id">
            <td>
              <NuxtLink :to="`/lat/admin/users/${msg.senderId}`">
                {{ msg.senderDisplayName }}
              </NuxtLink>
            </td>
            <td>
              <NuxtLink :to="`/lat/admin/users/${msg.recipientId}`">
...
async function fetchMessages() {
  try {
    loading.value = true
    error.value = ''
    const response = await $fetch<FlaggedMessagesResponse>('/apiv2/lat/admin/messages/flagged')
...
async function reviewMessage(msgId: number, approve: boolean) {
  ...
  try {
    await $fetch(`/apiv2/lat/admin/messages/${msgId}/review`, {
      method: 'POST',
      body: { approve },
    })
