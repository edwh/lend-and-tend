<template>
  <div>
    <h2 class="lat-admin-title">Flagged Members</h2>

    <p class="lat-admin-subtitle">
      Members flagged for review or moderation.
    </p>
    ...
    <div class="lat-admin-card">
      <p v-if="!loading && members.length === 0" class="lat-admin-muted">
        No flagged members.
      </p>
      <table v-else-if="members.length" class="lat-admin-at">
        <thead>
          <tr>
            <th>ID</th>
            <th>Name</th>
        ...
        <tbody>
          <tr v-for="member in members" :key="member.id">
            ...
            <td>
              <span class="lat-admin-badge lat-admin-b-flagged">Flagged</span>
            </td>
            <td>
              <NuxtLink
...
useHead({ title: 'Flagged Members — L&T Admin' })
...
async function loadMembers() {
  ...
  try {
    ...
    const data = await $fetch(
      `${config.public.APIv2}/groups/${groupid}/members`,
      {
        params: { limit: 200, status: 'flagged' },
        headers: { Authorization: `Bearer ${authStore.auth.jwt}` },
      }
    )
    members.value = Array.isArray(data) ? data : data?.members ?? []
  } catch {
    error.value = 'Failed to load flagged members.'
  }
  loading.value = false
}
