      ...
      <table class="table table-striped">
        ...
        <tbody>
          <tr v-for="agreement in filteredAgreements" :key="agreement.id">
            <td>
              <NuxtLink :to="`/admin/users/${agreement.lenderId}`">
                {{ getLenderName(agreement.lenderId) }}
              </NuxtLink>
            </td>
            <td>
              <NuxtLink :to="`/admin/users/${agreement.tenderId}`">
...
async function fetchAgreements() {
  try {
    ...
    const response = await $fetch<AgreementsResponse>('/apiv2/admin/agreements')
    ...
    for (const userId of userIds) {
      if (!userCache.has(userId)) {
        try {
          const user = await $fetch<{ displayName?: string }>(`/apiv2/admin/users/${userId}`)
