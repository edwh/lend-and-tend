...
export const useLatAgreementsStore = defineStore('latAgreements', () => {
  ...
  const fetchMyAgreements = async () => {
    ...
    error.value = null
    try {
      const response = await fetch('/apiv2/lat/agreement/my')
      if (!response.ok) {
  ...
  const createAgreement = async (lenderId: number, tenderId: number) => {
    ...
    error.value = null
    try {
      const response = await fetch('/apiv2/lat/agreement', {
        method: 'POST',
  ...
  const fetchAgreement = async (id: number) => {
    ...
    error.value = null
    try {
      const response = await fetch(`/apiv2/lat/agreement/${id}`)
      if (!response.ok) {
  ...
  const agreeToAgreement = async (id: number) => {
    ...
    error.value = null
    try {
      const response = await fetch(`/apiv2/lat/agreement/${id}/agree`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      })
      if (!response.ok) {
  ...
  const getAgreementPDF = async (id: number) => {
    try {
      const response = await fetch(`/apiv2/lat/agreement/${id}/pdf`)
      if (!response.ok) {
